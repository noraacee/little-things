...
public class SystemMessageManager {
    ...
    public void displayError(String errorMessage) {
        systemMessage.setText(errorMessage);
        setErrorBackground();
    }

    public void displayErrorFill(int errorCode, String... fill) {
        String errorMessage = errorFillMap.get(errorCode);
        for (int i = 0; i < fill.length; i++) {
            String replacer = "";
            for (int j = i; j < fill.length; j++) {
                replacer += "*";
            }
            errorMessage = errorMessage.replace(replacer, fill[i]);
        }
